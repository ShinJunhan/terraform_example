# terraform_example/test02_basic/example2/main.tf
...
resource "aws_instance" "my_ec2" {
    # ec2 인스턴스 3개 만들기 (aws_instance.my_ec2[] 은 배열)
    count                   = 3
...
output "instance_public_ip"{
    ...
    # * 연산자를 이용해서 모든 public ip 를 배열에 담아오기
...
resource "terraform_data" "wait_for_instance"{
    # 서버, 인벤토리, 설정 파일이 모두 준비된 이후에 이 블럭이 실행되도록 순서 보장 (실행순서만 보장)
    ...
    # 즉 ec2 가 새롭게 만들어지면 이블럭이 다시 실행되고 결과적으로 sleep 60 이 다시 실행된다.
...
resource "terraform_data" "ansible_run"{
    ...
    # ec2 가 새롭게 만들어지면 ansible playbook 을 실행하도록 여기에도 방아쇠를 걸어 놓는다
    triggers_replace = aws_instance.my_ec2[*].id
