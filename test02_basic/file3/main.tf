# ~/terraform_example/test02_basic/file3/main.tf
...
output "debug01" {
    # yamlencode() 함수를 이용하면 map 에 들어 있는 데이터를 참조해서 yml 문서를 만들어낼 수 있다
    # ansible 에서 사용하는 inventory 파일을 yml 형식으로 정의할 수도 있는데
        ...
        all = {
            hosts = {
                "${var.ec2_ip}" = {
                    ...
                    ansible_ssh_private_key_file    = "${path.module}/lecture-key"
                }
            }
        }
    })
