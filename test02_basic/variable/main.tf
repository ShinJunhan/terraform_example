...
variable "instance_count" {
    ...
    # 숫자 type 3 을 기본값으로 지정
    default     = 3
}

# 3. List (배열 type)
variable "avail_zones" {
    # list 는 배열이고 list(string) 은 문자열이 들어 있는 배열을 의미한다
    type        = list(string)
    description = "사용할 가용영역 리스트 입니다"
    # 배열의 0번방, 1번방, 2번방에 문자열 넣어두기
    # 외부에서 참조할때는  var.avail_zones[0], var.avail_zones[1], var.avail_zones[2]
    # 와 같은 형식으로 참조 할수 있다.
...
variable "is_production" {
    type = bool
    description = "운영환경이면 true, 개발환경이면 false 를 넣으세요"
    default = false
}
...
output "debug03_info" {
    # 문자열 보간법을 이용해서 원하는 문자열 형식을 만들어서 출력할수 있다.
...
output "debug06_list_all" {
    # join() 함수를 이용해서 배열에 저장된 item 을 , 로 구분해서 합칠수 있습니다.
