# ~/terraform_example/test05_tailscale/main.tf
...
terraform {
  ...
  backend "s3" {
    # 미리 생성한 s3 버킷 이름
    bucket = "tfstate-bucket-b56e3612"
...
resource "aws_instance" "my_ec2" {
    ...
    # (추가) 다른 ec2 에서 mgmt 쪽으로 보내는 traffic 을 전달(routing)할 수 있도록 
    # (추가) 대장 ec2 에서 작업 > 네트워킹 > 소스/대상 확인 변경 > 소스/대상 확인 '중지' 체크
    source_dest_check = false # 이 설정을 false 로 해야 라우팅 기능 수행 가능
    
    tags = {
        Name = "my-ec2"
    }

    # (추가) ec2 가 provision 될 때, 실행할 shell script 를 지정할 수 있다
    user_data = <<-EOF
        #!/bin/bash
...
resource "terraform_data" "ansible_run"{
    ...
    provisioner "local-exec" {
        ...
        # (추가) --extra-vars 'host_name=${var.host_name}' 로 ansible 실행 시 동적으로 변수값 전달
        command = "ANSIBLE_SSH_PIPELINING=1 ansible-playbook site.yml --extra-vars 'host_name=${var.host_name}' "
    }
}

# (추가) host 명으로 사용할 변수를 선언하고 기본값 전달
variable "host_name" {
    type = string
    default = "db-server"
}
